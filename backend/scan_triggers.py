...
def juice_floor_block(net_juice_weekly_pct: float | None,
    ...
    floor returns None here (never blocks) — the share-denominated replacement is
    ``shadow_floor`` below, which EVALUATES and LOGS without ever blocking. This
    function is the BLOCKING one, so in shares mode it must stay silent."""
...
# list that feeds ``compose_row_verdict`` — that list is what gives a finding
# verdict authority, so keeping the shadow observation OUT of it is the load-
# bearing invariant of this whole feature. There is deliberately NO config switch
# that would turn any of this into a block; graduating a floor to blocking
# authority is a future work item contingent on logged real-data calibration, and
# it must be a deliberate code change reviewed on its own.
