...
def test_open_trades_are_excluded_from_the_count(tmp_store):
    store.save_trades("2026-09-14", {"t1": _trade("t1", status="open", closed_at=None)})
...
def test_verdict_is_win_loss_or_flat_only_once_complete(tmp_store, monkeypatch):
    ...
    store.save_trades("2026-09-14", {"t1": _trade("t1", realized_pnl=50.0, closed_at="a")})
    assert trial.trial_status()["verdict"] == "win"

    store.save_trades("2026-09-15", {"t2": _trade("t2", realized_pnl=-50.0, closed_at="b")})
...
def test_a_losing_trial_verdicts_loss(tmp_store, monkeypatch):
    ...
    assert trial.trial_status()["verdict"] == "loss"


def test_a_breakeven_trial_verdicts_flat(tmp_store, monkeypatch):
    monkeypatch.setattr(config, "DAYTRADE_TRIAL_TRADES", 1)
    store.save_trades("2026-09-14", {"t1": _trade("t1", realized_pnl=0.0, closed_at="a")})

    assert trial.trial_status()["verdict"] == "flat"
