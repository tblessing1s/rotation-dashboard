...
# Juice viability floor — the SAFETY block the canonical scan verdict enforces
# (low IV does not fix itself on a date, so a sub-floor name is BLOCKED, never
# benchable). Two tiers:
#   * hard floor  — NET juice/wk (post LEAP-burn + slippage) <= 0: burn exceeds
#     income, not a trade at any conviction. Always applies; no constant.
#   * adequacy floor — GROSS juice/wk (weekly short extrinsic / LEAP cost, BEFORE
#     burn) below JUICE_FLOOR_WK: the premium itself is too thin. Keyed off GROSS
#     because that is the strategy's stated income bar (~1.9%/wk, the HARD_CFM_RULE
#     CYCLE_RETURN_MIN/CYCLE_WEEKS_MAX); the net column still shows the take-home.
...
# weekly; the current sector universe stays operative UNLESS the screen is enabled.
UNIVERSE_SCREEN_ENABLED = os.environ.get("CFM_UNIVERSE_SCREEN", "").strip().lower() in ("1", "true", "yes")   # default OFF — shadow artifact
CANDIDATE_UNIVERSE_CHANGELOG_MAX = 2000    # PROPOSED_DEFAULT — retained add/drop entries
