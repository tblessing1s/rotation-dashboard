...
ALERT_TYPES = {
    ...
    "ORDER_FILLED_DURING_CANCEL": ("CRITICAL", "HARD_CFM_RULE: an order filled while being cancelled -> the position is unexpectedly LIVE; reconcile the fill, do not resubmit, confirm delta coverage"),
...
def check_payout_ready(state: dict) -> list[dict]:
    ...
    repaid = pending.get("intrinsic_repaid") or 0
    parts = []
    if burn:
        parts.append(f"${burn:,.2f} LEAP burn")
    if repaid:
        parts.append(f"${repaid:,.2f} intrinsic repaid")
    breakdown = (f" (${pending['net_juice']:,.2f} juice − " + " − ".join(parts) + ")"
                 if parts else "")
    a = _alert(
        "PAYOUT_READY", None,
        ...
        {"month": pending["month"], "net_juice": pending["net_juice"],
         ...
         "net_payout": pending["net_payout"], "reason": pending["reason"]},
        key=pending["month"])
