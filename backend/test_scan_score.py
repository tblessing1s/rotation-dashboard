...
def test_score_is_bounded_0_to_10():
    lo = _score(inst_flow=InstFlow.DISTRIBUTING, base_stage=BaseStage.DECLINING, sector_rs1m=-20.0, atr_momentum=2.0,
                pct_above_ma21=40.0, net_juice_weekly_pct=0.0)
    hi = _score(inst_flow=InstFlow.ACCUMULATING, base_stage=BaseStage.EARLY_ADVANCE,
                base_count=1, sector_rs1m=10.0,
...
def test_worst_case_is_zero():
    assert _score(inst_flow=InstFlow.DISTRIBUTING, base_stage=BaseStage.DECLINING, sector_rs1m=-20.0, atr_momentum=2.0,
...
def test_accumulating_outscores_early_interest_all_else_equal():
    common = dict(base_stage=BaseStage.EARLY_ADVANCE,
...
# test_turning_outscores_fading_all_else_equal was DELETED 2026-08-21: it
# ordered the two-speed RS-vs-SECTOR states, and that component was removed from
# the SCORE with the rest of the sector-relative logic
# (docs/decision-2026-08-21-remove-sector-rs.md).


def test_extended_from_ma21_scores_lower_than_near():
    common = dict(inst_flow=InstFlow.ACCUMULATING, base_stage=BaseStage.EARLY_ADVANCE, sector_rs1m=5.0, atr_momentum=0.9,
                  net_juice_weekly_pct=2.0)
...
def test_missing_inputs_are_neutral_not_crashing():
    ...
    assert set(out["parts"]) == {"inst_flow", "base", "sector",
                                 "atr", "dist_ma21", "net_juice", "juice_viability"}
    assert "rs_state" not in out["parts"]      # removed with the sector RS
...
def test_juice_viability_clamps_at_zero_for_nonpositive_and_full_above_target():
    quality = dict(inst_flow=InstFlow.ACCUMULATING, base_stage=BaseStage.EARLY_ADVANCE, sector_rs1m=5.0, atr_momentum=0.9,
                   pct_above_ma21=3.0)
...
def test_parts_are_reported_for_the_calibration_log():
    out = scan_score.compute_score(
        ...
        base_count=1, sector_rs1m=5.0, atr_momentum=0.9,
